...
export default function DayDetail() {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <h1 className="text-lg font-semibold">Day details</h1>
      <p className="text-xs text-gray-500">{niceDate}</p>

      <div className="grid grid-cols-2 gap-3 text-center text-xs">
        <div className="bg-blue-50 p-3 rounded-lg">
          <p className="text-gray-500 uppercase tracking-wide">Calories In</p>
          <p className="text-lg font-bold mt-1">{totals.in}</p>
        </div>
        <div className="bg-orange-50 p-3 rounded-lg">
          <p className="text-gray-500 uppercase tracking-wide">Calories Out</p>
          <p className="text-lg font-bold mt-1">{totals.out}</p>
        </div>
      </div>

      <div className="bg-white rounded-lg shadow-sm p-3 text-xs space-y-2">
        <h2 className="font-semibold text-sm">Macros vs targets</h2>
      ...
      <div className="space-y-3 text-xs">
          ...
          {dayMeals.length === 0 ? (
            ...
            <div className="space-y-2">
              {dayMeals.map((m) => (
                <div
                  key={m.id}
                  className="bg-white rounded-lg shadow-sm p-2 flex justify-between gap-2"
                >
                  <div>
        ...
        <div>
          ...
          {dayWorkouts.length === 0 ? (
            <p className="text-gray-400">No workouts logged.</p>
          ) : (
            <div className="space-y-2">
              {dayWorkouts.map((w) => (
                <div
